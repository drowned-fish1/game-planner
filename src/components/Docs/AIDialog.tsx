// src/components/Docs/AIDialog.tsx
...
interface AIDialogProps {
  mode: AIMode;
  selectedText?: string; // 选中的原文（如果是生成模式则为空）
...
export function AIDialog({ mode, selectedText = '', onInsert, onClose }: AIDialogProps) {
  ...
  // 初始化：如果是润色/扩写，自动触发；如果是生成，等待用户输入指令
  useEffect(() => {
  ...
  const handleRunAI = async (inputContent: string) => {
    ...
    if (mode === 'generate') {
       ...
       user = `原文：\n"${selectedText}"\n\n请执行${mode}操作。`;
       // 如果用户有额外指令（比如“翻译成日文”），可以在界面加个输入框追加
    }
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-black/60 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
      <div className="bg-slate-800 border border-slate-700 w-[700px] max-h-[85vh] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-6">
          ...
          {/* 1. 输入区域 (生成模式显示输入框，其他模式显示原文预览) */}
          <div className="space-y-2">
             <label className="text-xs font-bold text-slate-500 uppercase">
                {mode === 'generate' ? '输入指令 (Prompt)' : '原文内容 (Original Context)'}
             </label>
             ...
             {mode === 'generate' ? (
               <div className="flex gap-2">
                 <input 
                   ...
                   placeholder="例如：设计一把传说级的火焰长剑，包含属性和背景故事..."
                   className="flex-1 bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-sm text-white focus:border-purple-500 outline-none"
                   autoFocus
                 />
                 <button 
                   onClick={() => handleRunAI(prompt)}
                   disabled={loading || !prompt.trim()}
                   className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white px-4 rounded-lg font-bold"
                 >
                   <ArrowRight size={18} />
                 </button>
               </div>
          ...
          {(loading || result || error) && (
            <div className="space-y-2 animate-in fade-in slide-in-from-bottom-2">
               <div className="flex justify-between items-center">
                 <label className="text-xs font-bold text-emerald-500 uppercase">AI 生成结果 (Result)</label>
                 {result && (
                   <div className="flex gap-2">
               ...
               <div className={`p-4 rounded-lg border min-h-[100px] text-sm leading-relaxed whitespace-pre-wrap ${
             ...
             <>
               {mode !== 'generate' && (
                 <button 
                   ...
                   className="px-4 py-2 bg-red-600/80 hover:bg-red-500 text-white rounded-lg text-sm font-bold flex items-center gap-2"
                   title="直接替换选中的原文"
                 >
                   替换原文
                 </button>
               ...
               <button 
                 onClick={() => onInsert(result, 'insert')} 
                 className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-emerald-900/20"
                 title="在光标处/原文下方插入"
               >
                 <Check size={16} /> 确认插入
               </button>
